...
| Recap never arrives | First: was a session actually closed? `journalctl -u gamegate \| grep "session closed"`. The recap is delivered by the **desktop app**, which polls `/digests` — so check the tray app is running and reaching the API. Discord is only a secondary surface (the bot's `!digest` command pulls the latest on demand; it is not the primary delivery). Also note: under the current recap scoping, a recap only contains messages that arrived *during* that game — messages held while away/focused live in the dashboard Messages tab, not a recap. |
