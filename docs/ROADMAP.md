...
| **Easy connectors** | Interactive Connectors tab + Add-a-connector catalog EXIST | Per-app OAuth flows behind the "Connect" buttons (Gmail flow already built as the template); Windows notification-listener catch-all — code complete, but BLOCKED on app packaging: Windows only grants notification-read access to a registered/installed app (MSIX or AUMID+shortcut), not a bare .exe, so the listener stays inert until GameGate ships as an installed app |
